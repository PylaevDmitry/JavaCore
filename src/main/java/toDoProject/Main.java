...
//        var ui = new TelegramBotUserInterface(env.get("BotToken"));
        ...
        while (userInput.length()==0 || Arrays.stream(invalidSymbols).anyMatch(userInput::contains)) {
            userInput = ui.askInput("Введите имя");
        }
//        var storage = new DBStorage(env.get("dbUserName"), env.get("dbUserPass"), userInput);
    ...
    static int getIndex (String userInput, Map<Integer, Task> map) {
        try {
            ...
            if (ValueRange.of(1, map.size()).isValidIntValue(taskIndex)) return taskIndex;
            return -1;
